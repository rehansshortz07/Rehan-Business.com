/* =========================================================
   MedLedger — synthetic patient database + render engine
   50 fictional patients, seeded so data is stable on reload.
========================================================= */
...
function ensurePatientSelect(){
  ...
  sel.id = 'patientSelect';
  sel.style.cssText = 'padding:8px 12px;border-radius:8px;border:1px solid var(--line);font-family:inherit;font-size:12.5px;background:#fff;margin-right:10px;';
  chip.parentElement.insertBefore(sel, chip);
  return sel;
}
...
function renderRecentActivity(p){
  let el = document.getElementById('recentActivity');
  if(!el){
    const dash = document.getElementById('dashboard');
    el = dash ? dash.querySelector('.timeline') : null;
  }
  if(!el) return;
...
function renderSummary(p){
  let el = document.getElementById('summaryText');
  if(!el){
    const summarySection = document.getElementById('summary');
    el = summarySection ? summarySection.querySelector('.summary-text') : null;
  }
  if(!el) return;
...
function initPatientSelector(){
  const sel = ensurePatientSelect();
  if(!sel) return;
  sel.innerHTML = PATIENT_DB.map(p => `<option value="${p.id}">${p.name} — ${p.id}</option>`).join('');
  sel.addEventListener('change', () => {
    const p = PATIENT_DB.find(x => x.id === sel.value);
